...
description: |
  ...
  Use when the user asks Claude or Codex for academic paper figures, thesis diagrams, architecture diagrams,
  flowcharts, TikZ/LaTeX diagrams, draw.io academic figures, or recreating a paper figure from text/image input.
  中文触发：论文配图、画架构图、画流程图、TikZ 图、draw.io 学术图、复刻论文图。
---
...
- 本 Skill 可由 Claude 或 Codex 执行；不要依赖 Claude 专属 UI/工具名。
- 当文中提到 Read/Write/Edit、AskUserQuestion、agent / sub-agent、视觉审查 agent 等 Claude Code 习惯用语时，Codex 应映射为当前环境等价能力：读取/编辑本地文件、必要时直接询问用户、在未显式授权并行 agent 时由主 agent 完成同等审查。
- `references/` 路径均相对本 `SKILL.md` 所在目录；Codex 执行脚本时先定位到 skill 根目录，或传入正确相对/绝对路径。
- 依赖检测、validator、linter、PDF overlap checker、打包/校验等步骤优先使用本地 shell 命令；缺少大型系统依赖时报告安装建议，不静默跳过质量门。
...
**Claude / Codex are capable of extraordinary academic figure design.** Checklist 是 catching last-mile bugs，**不是 safe defaults**。
...
**Batch 17 fig153 教训**：Philosophy 文本指南让 agent / sub-agent **知道要嵌入 viz / panel / 公式**，但**写出来的视觉重量、留白、配色协调仍然失败**——因为这些是 visual perception 而非 textual 任务。

**解决方案**：`references/tikz-snippets/` 提供 **21 个手工精雕的 TikZ 片段 + 21 个 PNG 预览 + 1 个组合规则文档**——agent / sub-agent **看 PNG 选 + 复制粘贴 + 替换参数 + 按组合规则拼装**即可达到 examples 标杆。
...
🔴 **工具铁律：只用 TikZ 或 draw.io，禁止 Python/matplotlib 替代**（2026-05-22 Batch 17 fig153 教训：agent / sub-agent 在执行 Module-First 时用 Python+matplotlib 生成 `.py` 文件 = 完全偏离 thesis-figure-skill 价值主张）：
...
- agent / sub-agent 不要自作主张套 dark theme
...
**🔴 强制物质化要求**（fig126 Tacotron2 教训：agent / sub-agent 加载 step1 但跳过实际输出，产生大块空白）：
...
**写在 figure.tex 头部作为注释**，agent / sub-agent 在 ④.5 Step 0 时读取核验。若注释不存在 → blocker 回 ① 重做。**注意**：form B 不是必须填模板，是用"设计师的叙述思维"展开布局——重点是 Philosophy 段的 **UNFORGETTABLE Question**：你怎么布局让审稿人 5 秒记住一个独特结构？

### ①.5 图档判断（**用户驱动** + 自动检测兜底）

**🔴 复杂度按需而定，不是默认复杂**（Batch 16 教训：Philosophy First 让 agent / sub-agent 默认套 examples 06 复杂风格，但用户实际只想要中等清晰图时 = 过度发挥变乱）。
...
```
用户原 prompt 没明示复杂度时，Claude 可使用 AskUserQuestion；Codex 或普通 CLI 环境直接询问用户：
...
**🔴 复杂档强制 Module-First 子流程**（Batch 16 教训：agent / sub-agent 一次写 800 行 TikZ → 整图乱；先画一部分，验证好，再拼接 = 干净）：
...
→ Yes：**B 路（auto-layout）**
  ...
  - **优势**：0 个模型凭感觉选的坐标，0 类微斜线/穿框/拥挤 bug
...
- `python3 references/tikz-design-linter.py <file.tex> [--type rich|sequence|simple]` — **advisory** 设计指标。**默认 WARN 不阻挡**——简单论文图可以简单，linter 只汇报指标供 agent+用户判断是否匹配论文复杂度。仅当 ≥8 节点且尺寸比 < 1.5（严重视觉层次缺失）才 ERROR。
...
# overlap.json 落在 .tex 同目录（agent / sub-agent ④.5 步读取这个绝对路径）
...
# / panel 边界等常误报），agent / sub-agent 在 ④.5 自评时按坐标对照 PNG 决定 fix 还是 ignore。
```
...
```
0. 读取/查看渲染出的 out-1.png + 视觉直觉先行（应用 3 大法则，见下）
1. 读取 overlap.json（路径 = .tex 同目录的 overlap.json，步骤 ④ 跑出来的结构化几何检测）
...
**为什么 Step 0 在 18 项之前**：18 项是机械验证（细节体检），容易陷入"逐项 Y / 整体烂"。Step 0 是视觉直觉（整体心电图），强迫 agent / sub-agent **以读者视角看图**而不是 generator 视角。两者缺一不可。
...
**优化规则**（Batch 9 实测：fig89 耗 32min / fig86 耗 55min，多数时间在重复读取和 triage）：

1. **Reference 文档只首轮加载** — `SKILL.md` / `lessons.md` / `visual-patterns.md` / `step1-instructions.md` / `visual-review-checklist.md` 在 round 1 读取一次后，**后续 round 不要重复读取**（context 里已有）。只读取变化的：PNG + overlap.json + 你刚 Edit 的 figure.tex 片段
...
1. **Reference docs 只在 round 1 读取一次** —— `SKILL.md` / `lessons.md` / `visual-patterns.md` / `step1-instructions.md` / `visual-review-checklist.md` 在 round 1 加载到 context 后，后续 round **不要再读取**——已经在 context 里。**仅 PNG / overlap.json / figure.tex 每轮读取（这些会变）**。
...
- 复杂图（≥40 元素）/ **mode C 一律**：环境支持且已授权并行 agent 时，spawn 多镜头审查 agent（见下「mode C 独立多镜头审查 gate」），统一收集 blocker；若当前 Codex/CLI 环境不允许 spawn，则由主 agent 按 4 个镜头分轮独立记录审查结论，并在交付中说明该降级。
- **自评 + 对抗 agent 全部说 0 blocker 后，仍然把图先给用户看**——人眼能发现 agent / sub-agent + 自评全漏掉的问题（实测案例：用户在 Round 10 一眼指出 CMAM 标题切断边框、箭头刺入框内、junction dot 被 tip 戳——这 3 个我和 2 个对抗 agent 全没发现）。**用户的视觉是最后闸门，不是绕过点**。
...
#### mode C 独立多镜头审查 gate（2026-05-29 mode C 测试坐实；不可静默跳过）

**为什么需要独立审查**：mode C 测试（同心圆 / Sankey / RLHF 闭环，3 张从零图）实测 **3/3 都崩，且每次崩在不同轴上**，但有统一规律——**图里最"新"、最无 skeleton 先例的那个结构元素，就是崩点**：
...
**所以 mode C 的 ④.5 不允许只靠 generator 自审**。Step 0 + 18 项照走，但**完成后必须再过这道 gate**：优先由 orchestrator **独立 spawn 4 个全新审查 agent**（各自空白上下文），各自读取渲染出的 PNG，**对抗式**找问题（默认"图里有 bug，去找"），各自返回 blocker（含 severity + 位置 + **具体视觉证据**，证明它真看了图）。如果当前环境没有并行 agent 能力或用户/系统未授权 spawn，主 agent 必须按 geometry/semantic/text/aesthetic 四个镜头分轮审查，逐项写出证据；这属于降级路径，不能假装等同于独立 gate：
...
**gate 之后仍不能直接交付**：
1. **你（orchestrator）必须亲自读取/查看 PNG**——多镜头 agent 也有盲区。实测：orchestrator 自己修完一张图后亲眼自审，仍漏掉了自己引入的"公式出框"回归，是再跑一轮独立审查才抓到。**本节"用户的视觉是最后闸门 / 单视角自评不可靠"对你（orchestrator）同样成立。**
...
**机制**：有 Agent 工具 / workflow 时，**真正并行 spawn 出 4 个独立子 agent**（每镜头喂 PNG 路径 + 该图"应有模型"让 semantic 独立重算）。没有该能力时，执行四镜头降级审查并明确标注 limitation。**关键是"独立视角 + 对抗 + 多证据"；同上下文随口自审多遍不算数（实测会连同 generator 一起漏）。**
...
Φ.5 — 自动生成 USAGE header（**含具体生成算法**）
   - 顶部模板字段及生成方法：
     ...
     * **USAGE**：固定模板 "Agent / sub-agent: copy ENTIRE file as figure.tex,
...
Φ.7 — 提醒用户后续补 CONSTRAINTS（关键防回归提示）
   - "⚠️ 新 skeleton 已入库，但 CONSTRAINTS 段当前是 TODO 占位。
      ...
      box 宽度依赖等），请手动补充到这一段。
      这是防止未来 agent / sub-agent 复用时重新引入 bug 的关键——参考
...
| 步骤③ 走 B 路 → 跑 `dot-to-tikz.py` | （脚本，不需读取） |
