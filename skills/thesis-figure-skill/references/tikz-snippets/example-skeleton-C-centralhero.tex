...
%
% Agent / sub-agent: replace titles + labels + numbers. Keep grid + zone rects.
% =====================================================================
...
% Fix: was rowH=0.27 with inner-pad 0.15 — labels too close to table
% border (cramped). rowH=0.28 gives 9*0.28=2.52cm tab height (fits zone
% y=[6.0, 9.5]) with 0.18cm zone padding. Inner-pad 0.15→0.30cm.
\def\xLU{0.4}\def\yLU{8.7}\def\rowH{0.28}\def\colW{8.7}
\def\tabdata{%
...
% Fix 4: heatmap was not horizontally centered in hero zone.
% Hero zone center x = 10.5. Heatmap width = N*cs = 2.56cm.
% So xH = 10.5 - 1.28 = 9.22 → heatmap center = 9.22 + 1.28 = 10.5 ✓
\def\xH{9.22}\def\yH{4.3}\def\cs{0.32}\def\N{8}
\foreach \i in {1,...,\N} {
  \foreach \j in {1,...,\N} {
...
% Label removed: above caused overlap with hero title, below caused
% overlap with zone bottom border. Heatmap visual is self-explanatory
% in context of "Core Architecture (Hero)" zone with adjacent formulas.

% Right: 2 formula boxes at x=[13.5, 20]
% Use \footnotesize for formulas — matches surrounding box label size
...
% Fix C.5: pipeline boxes were asymmetric (left 0.375cm vs right 0.075cm)
% Center 5 boxes in zone x=[0, 9.5]: total content 5*1.65 + 4*0.2 = 9.05,
% padding (9.5-9.05)/2 = 0.225 each side. First box center at 0.225+0.825=1.05
\foreach \i/\name/\col in {1/Input/acaTealLine, 2/Embed/acaBlueLine,
                          3/Encode/acaPurpleLine, 4/Decode/acaOrangeLine,
                          5/Output/acaGreenLine} {
  \pgfmathsetmacro\cx{1.05 + (\i - 1) * 1.85}
...
% =====================================================================
% Fix C.1: was {acaGreenLine}{acaGreenLine} → fill=#2!50 made solid
% saturated green that drowned the white metric cards. Use the Bg
% variant just like the other panels (LU/RU/Hero/LD).
\drawzone{acaGreenLine}{acaGreenBg}{11.0}{-2.5}{21.0}{1.0}
\zonetitle{acaGreenLine}{11}{21}{1.0}{Performance Metrics}

% Metric cards (use \large not \Large — matches surrounding box font)
