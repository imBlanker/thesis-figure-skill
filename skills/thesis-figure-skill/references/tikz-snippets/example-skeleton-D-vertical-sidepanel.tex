...
\draw[-{Stealth[scale=1.0]}, very thick, acaPurpleLine] (secagg.east) -- (dp.east |- secagg.east);

% Wait - secagg.east to dp.west
% Fix:
% Connection already handled, let me fix
% Actually the arrow should go from secagg.east to dp.west

% Decrypted output label
\node[font=\scriptsize, text=acaPurpleLine] at (9.5, 7.8) {Decrypted $\tilde{w}_{\text{global}}$};
\draw[arrow] (9.5, 8.2) -- (9.5, 7.5);

% ================================================================
%  SECURITY ANALYSIS (top-right, radar chart)
% ================================================================
\node[font=\small\bfseries, text=acaTealLine, anchor=north west] at (19.5, 19.5)
    {Security Analysis};
...
\begin{scope}[shift={(1.5, 0.5)}]
  ...
  \node[font=\tiny, text=acaGreyLine] at (3.0, -0.3) {Communication Rounds};
  % Y ticks
...
\begin{scope}[shift={(11.5, 0.5)}]
  ...
  % CKKS packing annotation
  \node[font=\tiny, text=acaTealLine, anchor=west] at (3.0, 3.2)
...
% ================================================================
%  INTER-ZONE ARROWS (dashed, clients → crypto)
% ================================================================
% Dashed border from client zone to crypto zone
\draw[acaBlueLine!40, thick, dashed, rounded corners=4pt]
    (0, 14.2) -- (0, 13.8);
\draw[acaBlueLine!40, thick, dashed, rounded corners=4pt]
    (18, 14.2) -- (18, 13.8);

% Fix the Step 2→Step 3 arrow (was wrong target)
