...
\begin{tikzpicture}[
    ...
    arrow short/.style={                             % 短箭头（< 1.5cm = 42pt）专用
        -{Stealth[length=3pt 1.0, width'=0pt 0.5, sep=0pt 0.3]},
        line width=0.8pt,                            % 比 arrow 细，tip 更紧凑
        shorten >=1pt, shorten <=0pt,                % shorten 减半，stem 保留
        color=black!70,
    },
    ...
    fan_stub/.style={                                % 树状分叉的 stub（spine→target）
        -{Stealth[length=5pt 1.5, width'=0pt 0.6, sep=0pt 0.5]},
        line width=1.0pt,
        shorten >=2pt,                               % tip 离 target 框 2pt
        shorten <=0pt,                               % ⚠️ 起点紧贴 spine——禁止 shorten 制造视觉断裂
        color=black!70,
    },
    fan_stub thin/.style={fan_stub, line width=0.6pt},
    fan_stub thick/.style={fan_stub, line width=1.6pt},
...
%
% 2. 树状分叉（spine + N stubs）— 关键：**spine 用普通 \draw（无 shorten）+ stubs 用 fan_stub 样式（shorten <=0pt）**
%
%    ✅ 正确写法（spine 连续，stub 起点紧贴 spine）：
%      % spine 一条线，无 arrow 无 shorten
%      \draw[line width=1.0pt, color=black!70] (left_x, spine_y) -- (right_x, spine_y);
%      % 每条 stub 用 fan_stub（shorten <=0pt，起点紧贴 spine 无 gap）
%      \draw[fan_stub] (stub1_x, spine_y) -- (target1.north);
%      \draw[fan_stub] (stub2_x, spine_y) -- (target2.north);
%      \draw[fan_stub] (stub3_x, spine_y) -- (target3.north);
%
%    ❌ 错误写法（stub 用 arrow 样式 → shorten <=1pt 造成 spine 和 stub 间 1pt 视觉断裂）：
%      \draw[arrow] (stub1_x, spine_y) -- (target1.north);  % 起点离 spine 1pt
%
%    ❌ 不要在 spine 上放 fill=white 的 sum/junction 圆圈，否则圆圈会**截断** spine
%      → 用 fill=（spine 同色）或单独把 spine 在圆圈位置打断再画
%
%    主干+横杆一笔画完的备选写法（更紧凑但只适合 1 source → 多 target）：
%      \draw[line width=1.0pt, color=black!70]
%          (A.south) -- ++(0,-1.5) coordinate(crossbar_mid)
%          (crossbar_mid) -| (B.north -| crossbar_mid)
%          (crossbar_mid) -| (C.north -| crossbar_mid);
%      \draw[fan_stub] (B.north -| crossbar_mid) -- (B.north);  % 用 fan_stub 收尾
%
